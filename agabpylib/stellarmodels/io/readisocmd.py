...
class MIST:
    ...
    def age_index(self, age):
        ...
        if (age > max(self.ages)) | (age < min(self.ages)):
            print(
                "The requested age is outside the range. Try between "
                + str(min(self.ages))
                + " and "
                + str(max(self.ages))
            )
...
class PARSEC:
    ...
    def read_isocmd_file(self):
        ...
        if self.colibri:
            ...
            abun = {
                "Z": float(content[dataStart + 1][0]),
                "[M/H]": log10(float(content[dataStart + 1][0]) / 0.0152),
            }
        else:
        ...
        for item in data:
            if comment.match(item[0]):
                if zstr.match(item[1]):
                    ...
                    if self.colibri:
                        formats = tuple(
                            [float64 for i in range(9)]
                            + [int32]
                            + [float64 for i in range(4)]
                            + [int32]
                            + [float64 for i in range(22)]
                        )
                    else:
                        formats = tuple(
                            [float64 for i in range(9)]
                    ...
                    isocmd = empty(
                        (0), {"names": tuple(hdr_list), "formats": tuple(formats)}
                    )
                continue
