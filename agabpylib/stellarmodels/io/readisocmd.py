...
class PARSEC:
    ...
    def __init__(self, filename, verbose=True, colibri=True):
        ...
        self.version, self.photo_sys, self.abun, self.Av_extinction, self.ages, self.num_ages, self.hdr_list, \
        self.isocmds = self.read_isocmd_file()
    ...
    def read_isocmd_file(self):
        ...
        Zsun = 0.0152
        avstring = re.compile('extinction')
        ...
        if self.colibri:
            if avstring.match(content[6][4]):
                dataStart = 12
                Av_extinction = float(content[6][6].split('=')[-1])
            else:
                dataStart = 11  # First line with table header
                Av_extinction = 0.0
            version = ' '.join(content[1][2:])
            photo_sys = ' '.join(content[5][4:])
            abun = {'Z': float(content[dataStart + 1][0]), '[M/H]': log10(float(content[dataStart + 1][0]) / 0.0152)}
        else:
            if avstring.match(content[5][4]):
                dataStart = 11
                Av_extinction = float(content[5][6].split('=')[-1])
            else:
                dataStart = 10
                Av_extinction = 0.0
            version = ' '.join(content[1][2:])
            photo_sys = ' '.join(content[4][4:])
            abun = {'Z': float(content[dataStart + 1][0]), '[M/H]': log10(float(content[dataStart + 1][0]) / 0.0152)}
            ...
            if comment.match(item[0]):
                if zstr.match(item[1]):
                    if not (isocmd is None):
                        isocmd_set.append(isocmd)
                        ages.append(isocmd[0][2])
                    hdr_list = item[1:]
                    if self.colibri:
                        formats = tuple([float64 for i in range(9)] + [int32] + [float64 for i in
                                                                                 range(4)] + [int32] + [float64 for i in
                                                                                                        range(22)])
                    else:
                        formats = tuple([float64 for i in range(9)] + [int32] + [float64 for i in range(9)])
