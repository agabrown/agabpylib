...
class hoyt(rv_continuous):
    ...
    .. math::
        ...
        f(x, a, b) = \frac{(1+a^2)x}{ab} \exp\left[-\frac{(1+a^2)^2 x^2}{4a^2b}\right]
                        I_0\left(\frac{(1-a^4)x^2}{4a^2b}\right)

    where :math:`I_0` is the modified Bessel function of the first kind of order zero
    (`scipy.special.iv <https://docs.scipy.org/doc/scipy/reference/generated/scipy.special.iv.html#scipy.special.iv>`_).
    The PDF is zero for :math:`x\leq0`.
    ...
    def _pdf(self, x, a, b):
        ...
        return (
            ...
            * x
            / (a * b)
            * np.exp(-((1.0 + a**2) ** 2) * x**2 / four_qsqrw)
    ...
    def _stats(self, a, b):
        ...
        skew = (
            -(
                np.sqrt(2)
                * (
                    np.pi * asqr * ellipk(oneminusasqr)
                    + ellipe(oneminusasqr)
        ...
        kurt = (
            4 * np.pi * asqr * (2 * ellipk(oneminusasqr) * ellipe(oneminusasqr) - np.pi)
            + 3 * np.pi**2 * oneplusasqr**2
            - 4
            * ellipe(oneminusasqr) ** 2
