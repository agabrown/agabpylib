...
def orbital_elements_to_xyz(a, e, incl, lon_nodes, argperi, tau, t):
    ...
    """
    if np.ndim(a) > 1 or np.ndim(e) > 1 or np.ndim(incl) > 1 or np.ndim(lon_nodes) > 1 or np.ndim(argperi) > 1 or \
            np.ndim(tau) > 1 or np.ndim(t) > 1:
        raise RuntimeError("Only scalars or 1D arrays allowed as input.")

    x = np.array([1, 0, 0])
    z = np.array([0, 0, 1])

    if np.ndim(a) + np.ndim(e) + np.ndim(incl) + np.ndim(lon_nodes) + np.ndim(argperi) + np.ndim(tau) + np.ndim(t) < 1:
        ...
        rvec_lmn = np.array([a * (np.cos(ecc_anomaly) - e), a * np.sqrt(1 - e * e) * np.sin(ecc_anomaly), 0])
    ...
    for index, aa, ee, ii, Omega, omega, mean_ano in zip(range(numbodies), semi_major, eccentricity, inclination,
                                                         longitude_nodes, arg_perihelion, mean_anomaly):
        ...
        rvec_lmn = np.array([aa * (np.cos(ecc_anomaly) - ee), aa * np.sqrt(1 - ee * ee) * np.sin(ecc_anomaly), 0])
