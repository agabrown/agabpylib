...
class UniformSpaceDistributionSingleLuminosity(ParallaxSurvey):
    ...
    def generateObservations(self):
        ...
        if not np.isinf(self.apparentMagnitudeLimit):
            low = (
                ...
                - 5 * self.stddevAbsoluteMagnitude
                + 5 * np.log10(self.minDistance)
                - 5.0
            )
            up = np.min(
                [
    ...
    def apparentMagnitude_lpdf(self, m):
        ...
        zH = (
            dmH - m + self.meanAbsoluteMagnitude - c * self.stddevAbsoluteMagnitude**2
        ) / (np.sqrt(2.0) * self.stddevAbsoluteMagnitude)
        zL = (
            dmL - m + self.meanAbsoluteMagnitude - c * self.stddevAbsoluteMagnitude**2
        ) / (np.sqrt(2.0) * self.stddevAbsoluteMagnitude)
        lpdf = (
            np.log(c)
            - np.log(a)
            ...
            * (
                m
                - self.meanAbsoluteMagnitude
                + 0.5 * c * self.stddevAbsoluteMagnitude**2
                + 5.0
            )
            + -np.log(2.0)
            + np.log(erf(zH) - erf(zL))
        )
...
class UniformDistributionSingleLuminosityHip(UniformSpaceDistributionSingleLuminosity):
    ...
    def _generateParallaxErrors(self):
        ...
        """
        errors = (
            self.parallaxErrorLogCalibrationFloor
            + self.parallaxErrorLogarithmicSlope
    ...
    def _generateApparentMagnitudeErrors(self):
        ...
        """
        errors = (
            self.magnitudeErrorLogCalibrationFloor
            + self.magnitudeErrorLogarithmicSlope
...
class UniformDistributionSingleLuminosityTGAS(UniformSpaceDistributionSingleLuminosity):
    ...
    def __init__(
        ...
        self.tgasErrorsPdfFile = path.join(
            _ROOT, "data", "TGAS-parallax-errors-pdf.csv"
        )
        self.tgasErrPdf = None
        if path.isfile(self.tgasErrorsPdfFile):
            self.tgasErrPdf = np.genfromtxt(
                self.tgasErrorsPdfFile,
                comments="#",
                skip_header=1,
                delimiter=",",
                names=["err", "logdens"],
    ...
    def _generateParallaxErrors(self):
        ...
        magnitude.
        """
        probs = np.exp(self.tgasErrPdf["logdens"]) / np.sum(
            np.exp(self.tgasErrPdf["logdens"])
        )
        errors = np.random.choice(
            self.tgasErrPdf["err"], size=self.apparentMagnitudes.size, p=probs
        )
        return errors
...
def showSurveyStatistics(simulatedSurvey, pdfFile=None, pngFile=None, usekde=False):
    ...
    goodParallaxes = (
        simulatedSurvey.observedParallaxes / simulatedSurvey.parallaxErrors >= plxSnrLim
    )
    estimatedAbsMags = (
    ...
    relParErr = (
        simulatedSurvey.parallaxErrors[positiveParallaxes]
        / simulatedSurvey.observedParallaxes[positiveParallaxes]
    )
    deltaAbsMag = (
        estimatedAbsMags - simulatedSurvey.absoluteMagnitudes[positiveParallaxes]
    )
    ...
    x = np.linspace(
        simulatedSurvey.minParallax,
        np.min([parLimitPlot, simulatedSurvey.maxParallax]),
        1001,
    )
    axA.plot(
        x,
    ...
    if usekde:
        ...
        samples = np.linspace(
            simulatedSurvey.trueParallaxes.min(),
            simulatedSurvey.trueParallaxes.max(),
            200,
        )[:, None]
    ...
    if usekde:
        ...
        samples = np.linspace(
            simulatedSurvey.observedParallaxes.min(),
    ...
    m = np.linspace(
        simulatedSurvey.observedMagnitudes.min(),
        simulatedSurvey.observedMagnitudes.max(),
        1000,
    )
    axB.plot(
        m, np.exp(simulatedSurvey.apparentMagnitude_lpdf(m)), "--", lw=3, label="model"
    )
    ...
    if usekde:
        ...
        samples = np.linspace(
            simulatedSurvey.apparentMagnitudes.min(),
        ...
        axB.hist(
            simulatedSurvey.apparentMagnitudes,
            bins="auto",
            density=True,
            histtype="step",
            lw=3,
    ...
    x = np.linspace(
        simulatedSurvey.absoluteMagnitudes.min(),
    ...
    if simulatedSurvey.absoluteMagnitudes[goodParallaxes].size >= 3:
        if usekde:
            ...
            bw = (
                1.06
                * scatter
                * simulatedSurvey.absoluteMagnitudes[goodParallaxes].size ** (-0.2)
            )
            kde = KernelDensity(bandwidth=bw)
